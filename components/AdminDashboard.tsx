...
/**
 * TEMPORARY DIAGNOSTIC — catches literally anything that goes wrong
 * anywhere on this page (not just inside try/catch blocks we already
 * wrote) and shows it directly on screen. This exists because mobile
 * Brave has no accessible console and there's no computer available to
 * remote-debug from. Safe to remove once "Generate keypair" is confirmed
 * working — it's not part of the app's real functionality, just a way to
 * see what a desktop browser's console would normally show for free.
 */
function GlobalErrorCatcher() {
  ...
  useEffect(() => {
    const onError = (e: ErrorEvent) => {
      setCaught((prev) => [...prev, `Error: ${e.message} (${e.filename}:${e.lineno})`]);
    };
    const onRejection = (e: PromiseRejectionEvent) => {
      const reason = e.reason instanceof Error ? e.reason.message : String(e.reason);
      setCaught((prev) => [...prev, `Unhandled promise rejection: ${reason}`]);
    };
  ...
  return (
    <div className="mb-4 space-y-1 rounded-lg border-2 border-red-500 bg-red-50 p-3 text-xs">
      <p className="font-bold text-red-700">DIAGNOSTIC — caught {caught.length} error(s):</p>
      {caught.map((msg, i) => (
        <p key={i} className="break-all font-mono text-red-800">{msg}</p>
      ))}
    </div>
  );
}
...
export function AdminDashboard() {
  ...
  if (!tokenConfirmed) {
    ...
    body = <Loader2 className="h-5 w-5 animate-spin text-muted" />;
  } else if (!privateKey) {
    body =
      bundle === null ? (
        <AdminSetup
    ...
    body = <AdminPanel privateKey={privateKey} adminToken={adminToken} />;
  }
